    ...
    <v-sheet width="100%" center justify-center>
      <v-slide-group v-model="model" show-arrows center-active>
        <v-slide-group-item v-for="card in cards" v-slot="{ toggle, selectedClass }">
          <v-card class="bg-primary" :class="['ma-sm-3 ma-2', selectedClass]" color="grey-lighten-1"
            :height="smAndUp ? 290 : 270" :width="smAndUp ? 200 : 180" rounded-lg @click="toggle">
            <v-img :src="card.src" :alt="card.title" class="d-flex align-end"
              gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.5)" height="100%" width="100%" cover>
              <v-card-title class="px-2 py-1 px-sm-4 py-sm-2 text-white text-sm-h6 text-subtitle-1 align-content-end"
